...
def test_ai_session_started_without_manual_session_creates_automatic_session(client: TestClient) -> None:
    response = client.post(
        "/api/detection-events",
        json={
            "post_id": 1,
    ...
    assert data["event"]["detected_game"] == "FC"
    assert data["session"] is not None
    assert data["session"]["status"] == "active"
    assert data["session"]["worker_id"] is None
    assert data["session"]["post_id"] == 1
    assert data["event"]["session_id"] == data["session"]["id"]


def test_ai_autonomous_workflow_per_match(client: TestClient, admin_token: str) -> None:
    # 1. AI detects session start on post 1 without auth
    ...
    assert start_data["session"]["worker_id"] is None
    assert start_data["session"]["status"] == "active"

    # 2. AI detects match ended
    ...
    assert match_data["session"]["id"] == session_id
    assert match_data["session"]["detected_match_count"] == 1
    assert match_data["session"]["expected_amount"] == 100

    # 3. An alert was created because detected_match_count (1) > declared_match_count (0)
    alerts_resp = client.get("/api/alerts", headers=auth_header(admin_token))
    assert alerts_resp.status_code == 200
    alerts = alerts_resp.json()
    assert any(a["session_id"] == session_id and a["worker_id"] is None and a["alert_type"] == "count_mismatch" for a in alerts)
