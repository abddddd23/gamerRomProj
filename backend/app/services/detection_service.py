...
def _create_underpaid_alert_if_needed(db: Session, session: GamingSession) -> None:
...
def create_detection_event(db: Session, payload: DetectionEventCreate) -> tuple[DetectionEvent, object | None]:
    ...
    game = resolve_ai_game(db, payload.game_type or payload.detected_game)
    ...
    if (
        ...
        _create_underpaid_alert_if_needed(db, session)
